									...
									{selectedModels.map((m) => {
										...
										return (
											<div key={m.id} className="h-full">
												<div
													className={`group flex flex-col gap-1 relative rounded-lg ${
														...
														isCollapsed
															? "cursor-pointer"
															: "hover:ring-white/20"
													} ${ranking?.rank === 1 ? "bg-gradient-to-t from-bg-yellow-500/10 to-yellow-400/10 ring-yellow-400/50" : ""}`}
													onClick={() => {
														if (isCollapsed)
															setCollapsedIds((prev) =>
																prev.filter((id) => id !== m.id),
												...
												>
													<div
														className={`flex items-center ${ranking?.rank !== 1 ? "justify-end" : "justify-between"}`}
													>
														{ranking && ranking.rank === 1 && !isCollapsed && (
															<div
																className={`bg-yellow-500/30 text-yellow-100 ring-yellow-400/50 inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold ring-1`}
															>
																<Crown size={11} />
																<span>BEST</span>
															</div>
													...
													>
														...
														!["Thinking…", "Typing…"].includes(
															String(ans.content),
														) ? (
															<>
																<div className="max-w-[72ch]">
																	<MarkdownLite
																		text={sanitizeContent(ans.content)}
																	/>
																</div>
																{/* Token usage footer */}
																{ans.tokens &&
																	...
																	(() => {
																		...
																		const inTokens = Array.isArray(
																			ans.tokens?.perMessage,
																		)
																			? ans.tokens!.perMessage!.reduce(
																					(sum, x) =>
																						sum + (Number(x?.tokens) || 0),
																					0,
																				)
																	...
																	ans.provider === "openrouter" && (
																		<div className="mt-2 inline-flex items-center gap-2 text-xs text-amber-200/90 bg-amber-500/15 ring-1 ring-amber-300/30 px-2.5 py-1.5 rounded">
																			<span>
																				...
																				Try again soon, switch model, or add
																				your own OpenRouter API key for higher
																				limits.
																			</span>
																			<button
																				onClick={() =>
